C****************************************************************************
C FILE: mpi_bandwidth.f
C DESCRIPTION:
C   Provides point-to-point communications timings for any even 
...
C     Change the next four parameters to suit your case
     ...
     &        ierr, status(MPI_STATUS_SIZE), namelength
...
C  few bandwidth calculations based upon message size and timings.           
      ...
      if (rank .lt. numtasks/2) then
        do n=start, end, incr
          ...
          do i=1, rndtrps
            t1 = MPI_WTIME()
            call MPI_SEND(msgbuf, n, MPI_CHARACTER, dest, tag, 
     &                    MPI_COMM_WORLD, ierr)
            call MPI_RECV(msgbuf, n, MPI_CHARACTER, src, tag, 
